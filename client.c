...
int start_client(int opcode, char *filename, int mode, char *server_addr, char *port)
{
    ...
    /*Data size*/
    int data_size = 0;
    long int rv_data_size = 0;
    long int sd_data_size = 0;
    ...
    if (is_read_request(opcode) || is_write_request(opcode))
    {
        ...
        if ((rv = getaddrinfo(NULL, CLI_PORT, &hints, &res)) != 0)
        {                                                            //do dns lookups and fetch all the possible interfaces
        ...
        fprintf(log, "Connection received from %s on port %d %s\n", server_addr, source_tid, get_time());
        ...
        if (is_read_request(opcode))
        {
            ...
            do
            {
                ...
                if (!strip_raw_packet(recv_buffer, reclength, pckt))
                {
                    ...
                    fprintf(log, "Malformed packet encountered. Exiting %s\n", get_time());
                }
                ...
                if (!is_data_packet(pckt->opcode))
                {
                    ...
                    else
                    {
                        ...
                        fprintf(log, "Unexpected packet encountered %s\n", get_time());
                    }
                }
            ...
            if (err_blck_tries == 0 || timeout_tries == 0)
            {
                ...
                fprintf(log, "Maximum tries reached %s\n", get_time());
            }
            ...
            while (data_size == 512)
            {
                ...
                while (reclength == -1 && timeout_tries > 0)
                {
                    ...
                    fprintf(log, "ACK sent: Block number = %d %s\n", pckt->block_number, get_time());
                ...
                if (pckt->block_number == (expected_blck_num + 1) % 65536)
                {
                    ...
                    rv_data_size = rv_data_size + data_size;
                    printf("Data packet of size %d and block number %d received\n", data_size, pckt->block_number);
                    fprintf(log, "Data packet of size %d and block number %d received %s\n", data_size, pckt->block_number, get_time());
                    copy_ack_header_to_buffer(expected_blck_num, send_buffer);
                }
                else
                {
                    ...
                    fprintf(log, "%d received %d expected %s\n", pckt->block_number, expected_blck_num, get_time());
                }
            }
            ...
            fprintf(log, "Sending the final ACK %s\n", get_time());
            copy_ack_header_to_buffer(pckt->block_number, send_buffer);
            if (send_packet(me, send_buffer, 4, otr, other_addrlen) == -1)
            {
                finish(fp, me);
                print_error("Unable to send packet");
                fprintf(log, "Unable to send packet %s\n", get_time());
            }
        if (rv_data_size >= 512){
            rv_data_size = rv_data_size + 512;
        }
        end = clock();
        printf("%ld bytes of data received\n", rv_data_size);
        fprintf(log, "%ld bytes of data received %s\n", rv_data_size, get_time());
        time_cost = (double)(end - begin)/CLOCKS_PER_SEC;
        printf("time cost to receive the data: %lf secs\n", time_cost);
        fprintf(log, "time cost to receive the data: %lf secs %s\n", time_cost, get_time());
        printf("data transfer speed: %.3f MB/s\n", rv_data_size/time_cost/1024/1024);
        fprintf(log, "data transfer speed: %.3f MB/s %s\n", rv_data_size/time_cost/1024/1024, get_time());
        }
        else
        {
            ...
            do
            {
                ...
                if (!strip_raw_packet(recv_buffer, reclength, pckt))
                {
                    ...
                    fprintf(log, "Malformed Packet %s\n", get_time());
                }
                ...
                if (!is_ack_packet(pckt->opcode))
                {
                    ...
                    fprintf(log, "Unexpected packet received %s\n", get_time());
                }
            ...
            while (data_size > 0)
            {
                ...
                //Receive the acks
                sd_data_size = sd_data_size + data_size;
                reclength = -1;
                ...
                while (reclength == -1 && timeout_tries > 0)
                {
                    ...
                    fprintf(log, "Sent data packet no. %d of size %d %s\n", expected_blck_num, data_size, get_time());
                    reclength = recv_packet(me, recv_buffer, otr, &other_addrlen);
                    if (reclength == -1)
                    {
                        print_data_wait_timeout(expected_blck_num);
                ...
                if (!strip_raw_packet(recv_buffer, reclength, pckt))
                {
                    ...
                    fprintf(log, "Malformed Packet received %s\n", get_time());
                }
                ...
                if (pckt->block_number == expected_blck_num)
                {
                    ...
                    fprintf(log, "Received ack for packet no. %d %s\n", pckt->block_number, get_time());
        ...
        printf("%ld bytes of data sent\n", sd_data_size);
        fprintf(log, "%ld bytes of data rsent %s\n", sd_data_size, get_time());
        time_cost = (double)(end - begin)/CLOCKS_PER_SEC;
        printf("time cost to send the data: %lf secs\n", time_cost);
        fprintf(log, "time cost to send the data: %lf secs %s\n", time_cost, get_time());
        printf("data transfer speed: %.3f MB/s\n", sd_data_size/time_cost/1024/1024);
        fprintf(log, "data transfer speed: %.3f MB/s %s\n", sd_data_size/time_cost/1024/1024, get_time());
        }
        finish(fp, me);
    }
    else
...
char *get_time(void)
{
    ...
    char *pure_time = asctime(timeinfo);
    char *sys_time = (char *)malloc(strlen(leftk) + strlen(pure_time) + strlen(rightk));
    if ((tmp = strstr(pure_time, "\n")))
    {
        *tmp = '\0';
    }
    sprintf(sys_time, "%s%s%s", leftk, pure_time, rightk);
    return sys_time;
}
